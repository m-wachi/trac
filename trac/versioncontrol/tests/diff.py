...
class DiffTestCase(unittest.TestCase):
    ...
    def test_delete_blank_line(self):
        opcodes = get_opcodes(['A', 'B', ''], ['A', 'B'], ignore_blank_lines=0)
        self.assertEqual(('equal', 0, 2, 0, 2), next(opcodes))
        self.assertEqual(('delete', 2, 3, 2, 2), next(opcodes))
        self.assertRaises(StopIteration, next, opcodes)

        opcodes = get_opcodes(['A', 'B', ''], ['A', 'B'], ignore_blank_lines=1)
        self.assertEqual(('equal', 0, 3, 0, 2), next(opcodes))
        self.assertRaises(StopIteration, next, opcodes)

        opcodes = get_opcodes(['A', 'B', ''], ['A'], ignore_blank_lines=0)
        self.assertEqual(('equal', 0, 1, 0, 1), next(opcodes))
        self.assertEqual(('delete', 1, 3, 1, 1), next(opcodes))
        self.assertRaises(StopIteration, next, opcodes)
    ...
    def test_space_changes(self):
        ...
        self.assertEqual(('equal', 0, 1, 0, 1), next(opcodes))
        self.assertEqual(('replace', 1, 2, 1, 2), next(opcodes))
        self.assertRaises(StopIteration, next, opcodes)
...
"""
        ...
        self.assertEqual(('equal', 0, 1, 0, 1), next(opcodes))
        self.assertEqual(('replace', 1, 6, 1, 5), next(opcodes))
        self.assertEqual(('equal', 6, 8, 5, 7), next(opcodes))
        self.assertRaises(StopIteration, next, opcodes)

        opcodes = get_opcodes(left, right, ignore_space_changes=1)
        self.assertEqual(('equal', 0, 1, 0, 1), next(opcodes))
        self.assertEqual(('delete', 1, 2, 1, 1), next(opcodes))
        self.assertEqual(('equal', 2, 4, 1, 3), next(opcodes))
        self.assertEqual(('replace', 4, 5, 3, 4), next(opcodes))
        self.assertEqual(('equal', 5, 8, 4, 7), next(opcodes))
        self.assertRaises(StopIteration, next, opcodes)
